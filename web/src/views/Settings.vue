      ...
      <div class="settings-section">
        ...
        <el-form label-width="120px" class="settings-form">
          <el-form-item label="主题配色">
            ...
            <div class="form-tip">选择面板的整体配色方案</div>
          </el-form-item>
    ...
    <div class="content-panel" v-if="activeTab === 'system'">
      <div class="settings-section">
        ...
        <el-form :model="systemForm" label-width="150px" class="settings-form">
          ...
          <el-form-item label="备份目录"><el-input v-model="systemForm.backupPath" /></el-form-item>
        </el-form>
      </div>
    </div>
...
const uiSettings = reactive({ theme: 'light', language: 'zh-CN' })
const systemForm = reactive({ panelName: 'UPanel', port: 8080, websitePath: '/www/wwwroot', backupPath: '/backup' })
...
.top-panel { background: #fff; border-radius: 4px; padding: 12px 16px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; }
.top-left { display: flex; gap: 8px; }
.top-right { flex-shrink: 0; }
.settings-tabs { margin-right: 16px; }
.content-panel { background: #fff; border-radius: 4px; padding: 20px; flex: 1; overflow-y: auto; }
...
.about-info { display: inline-block; text-align: left; background-color: #f9fafb; border-radius: 8px; padding: 20px 40px; margin-bottom: 24px; }
